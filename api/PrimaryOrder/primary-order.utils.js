...
} = require('../SuperStockist/super-stockist.utils');

const PRIMARY_ORDERS_AGGREGATE_QUERY = [
  {
    $lookup: {
      from: 'primaryorderskuitems',
...
const PRIMARY_ORDER_AGGREGATE_QUERY = [
  ...
  },
  {
    $project: {
      ...
      'sku_item.total_cost': '$sku_items.total_cost',
      'sku_item.pieces_per_carton': '$sku_items.pieces_per_carton',
      invoice_number: 1,
      customer_type: 1,
      super_stockist: 1,
      distributor: 1,
...
const updateCustomerInventory = async (order) => {
  if (order.customer_type === CUSTOMER_CONSTANTS.PRIMARY_DISTRIBUTOR) {
    return await incrementDistributorInventoryLevels({
      distributorId: order.distributor.id,
      skus: order.sku_items,
      primaryOrderId: order.id,
    });
  } else {
    return await incrementSuperStockistInventoryLevels(
      order.super_stockist.id,
      order.sku_items,
      order.id
    );
  }
